...
impl FallingGroundEditor {
    fn draw_event_view(&mut self, rect: Rect, current_ms: f32) {
        ...
        if !consumed_click_for_placement {
            ...
            self.event_hover_hint = Some(EventHoverOverlapHint {
                mouse_x: mouse.0,
                mouse_y: mouse.1,
                current_index,
        ...
        // 双击切换选中逻辑（与 note 一致）
            ...
            if ids.len() > 1 {
                ...
                if let Some(ref prev) = self.event_overlap_cycle {
                    if prev.col == col && prev.anchor_y == anchor_y && prev.candidates == ids {
                        let previous_in_current = ids
                            .iter()
                            .position(|id| *id == ids[prev.current_index.min(ids.len().saturating_sub(1))])
                            .unwrap_or(0);
                ...
                } else if let Some(ev) = self.timeline_events.iter().find(|e| e.id == ids[index]) {
        ...
        // 绘制悬停提示框
            if let Some(hint) = self.event_hover_hint {
                ...
                let metrics = measure_text(&text, self.text_font.as_ref(), hint_font_size, 1.0);
                let box_w = metrics.width + 14.0 * ui;
                let box_h = 24.0 * ui;
                let hx = (hint.mouse_x + 14.0 * ui).clamp(4.0 * ui, screen_width() - box_w - 4.0 * ui);
                let hy = (hint.mouse_y - box_h - 10.0 * ui).clamp(4.0 * ui, screen_height() - box_h - 4.0 * ui);
                draw_rectangle(hx, hy, box_w, box_h, Color::from_rgba(20, 24, 34, 214));
                draw_rectangle_lines(hx, hy, box_w, box_h, 1.0, Color::from_rgba(140, 156, 198, 220));
                draw_text_ex(
                    &text,
                    hx + 7.0 * ui,
                    hy + 17.0 * ui,
