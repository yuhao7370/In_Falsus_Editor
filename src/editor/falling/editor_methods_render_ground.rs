...
impl FallingGroundEditor {
    fn draw_ground_view(&self, rect: Rect, current_ms: f32, show_spectrum: bool) {
        ...
        if self.view.show_barlines {
            for barline in self.visible_barlines_cached(start_vb, end_vb) {
                ...
                let (thickness, mut color, alpha) = match barline.kind {
                    BarLineKind::Measure => (2.1, Color::from_rgba(170, 205, 255, 210), 210),
                    BarLineKind::Beat => (1.3, Color::from_rgba(112, 148, 192, 186), 186),
                    BarLineKind::Subdivision => (0.9, Color::from_rgba(80, 108, 142, 142), 142),
                };
        ...
        for note in &self.editor_state.notes {
            ...
            let head_y = self.time_to_y_from_metrics(note.time_ms, current_vb, judge_y, pixels_per_ms);
            ...
            if note.has_tail() {
                ...
                if y2 >= rect.y && y1 <= rect.y + rect.h {
                    let (body_x, body_w, body_color) = match note.kind {
                        GroundNoteKind::Hold => {
                            (note_x + note_w * 0.04, note_w * 0.92, palette.hold_body)
                        }
                        GroundNoteKind::SkyArea => {
                            (note_x + note_w * 0.02, note_w * 0.96, palette.skyarea_body)
                        }
                        _ => (note_x + note_w * 0.35, note_w * 0.3, palette.hold_body),
                    };
        ...
        let ground_label_metrics = measure_text(
            ground_label,
            self.view.text_font.as_ref(),
            ground_label_font_size,
            1.0,
        );
        draw_text_ex(
            ground_label,
