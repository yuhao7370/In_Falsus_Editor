...
impl FallingGroundEditor {
    fn update_hover_overlap_hint(
        ...
        if let Some(cycle) = &self.selection.overlap_cycle {
        ...
        self.selection.hover_overlap_hint = Some(HoverOverlapHint {
            mouse_x: mx,
            mouse_y: my,
            current_index,
    ...
    fn draw_overlap_hint(&self) {
        let Some(hint) = self.selection.hover_overlap_hint else {
            return;
        };
        if hint.total <= 1 {
            return;
        ...
        let metrics = measure_text(&text, self.view.text_font.as_ref(), font_size, 1.0);
        let box_w = metrics.width + 14.0 * ui;
        let box_h = 24.0 * ui;
        let x = (hint.mouse_x + 14.0 * ui).clamp(4.0 * ui, screen_width() - box_w - 4.0 * ui);
        let y =
            (hint.mouse_y - box_h - 10.0 * ui).clamp(4.0 * ui, screen_height() - box_h - 4.0 * ui);
